(() => {
  ...
  const elements=Object.fromEntries([
    'bond-total','bond-status','bond-search','bond-sort','show-curves','show-outliers','reset-filters','point-group','curve-group','grouping-notice','filter-grid','chart-title','chart-subtitle','curve-legend','canvas-wrap','bond-canvas','bond-tooltip','bond-rows','table-summary','page-prev','page-next','page-label','zoom-reset','residual-heading'
  ].map(id=>[id,document.getElementById(id)]));
  const state={
    bonds:[],date:'',metric:'yield_pct',selected:{rating:new Set(),industry:new Set(),ticker:new Set()},
    filterOrder:['rating','industry','ticker'],available:{},pointGroup:'band',curveGroup:'band',
    fit:new Map(),curves:new Map(),eligibility:new Map(),curveGroups:[],table:[],plot:[],page:1,
  ...
  function optionLabel(name,value,count){
    ...
    text.textContent=`${value} (${count.toLocaleString('en-US')})`;label.append(input,text);return label;
  }
  ...
  function syncCascadingFilters(){
    ...
    for(const name of state.filterOrder){
      const container=document.getElementById(`${name}-filter`),{values,counts}=state.available[name];container.replaceChildren();
      for(const value of values)container.append(optionLabel(name,value,counts.get(value)||0));
  ...
  function enforceGroupings(announce=false){
    ...
    const check=(kind,group)=>{
      ...
      const count=state.selected[group].size;
      if(count>=1&&count<=10)return group;
      if(announce)messages.push(`${kind}若依${groupLabels[group]}分類，請明確選取 1–10 項；已改回信評大類。`);
      return 'band';
    };
  ...
  function groupingValues(group,population){
    ...
    return sortedValues(state.selected[group]);
  }
  ...
  function computeCurves(){
    ...
    const key=[state.metric,state.curveGroup,selectedKey].join('::');if(key===state.curveKey)return;
    const population=curvePopulation(),groups=groupingValues(state.curveGroup,population);
    const result=model.buildCurves(population,state.metric,bond=>groupKey(bond,state.curveGroup),groups);
    state.curves=result.curves;state.fit=result.fitted;state.eligibility=result.eligibility;state.curveGroups=groups;state.curveKey=key;
  }
  ...
  function sortedTable(bonds){
    const mode=elements['bond-sort'].value,fit=bond=>curvePoint(bond)?.residual;
    ...
    const compare={
      'residual-desc':(a,b)=>numeric(a,b,fit,-1),'residual-asc':(a,b)=>numeric(a,b,fit,1),
      'yield-desc':(a,b)=>b.yield_pct-a.yield_pct,'oas-desc':(a,b)=>b.oas_bp-a.oas_bp,
      'maturity-asc':(a,b)=>a.maturity_years-b.maturity_years,'issuer-asc':(a,b)=>a.issuer.localeCompare(b.issuer),'ticker-asc':(a,b)=>a.ticker.localeCompare(b.ticker),
    }[mode];
  ...
  function tooltipHtml(bond){
    ...
    return `<strong>${escapeHtml(bond.security_des)}</strong><div>${escapeHtml(bond.issuer)} · ${escapeHtml(bond.ticker)}</div><dl><dt>Yield</dt><dd>${number(bond.yield_pct,3)}%</dd><dt>OAS Spread</dt><dd>${number(bond.oas_bp,2)} bp</dd><dt>Maturity</dt><dd>${escapeHtml(bond.maturity)} (${number(bond.maturity_years,2)}Y)</dd><dt>信評／產業</dt><dd>${escapeHtml(bond.rating)} · ${escapeHtml(bond.industry)}</dd><dt>點位顏色</dt><dd>${groupLabels[state.pointGroup]} · ${escapeHtml(groupKey(bond,state.pointGroup))}</dd><dt>曲線分類</dt><dd>${groupLabels[state.curveGroup]} · ${escapeHtml(curveGroup)}</dd><dt>${active} curve</dt><dd>${curve}</dd><dt>${active} − curve</dt><dd>${residual}</dd></dl>${flag}`;
  }
  ...
  function renderLegendGroup(title,group,mode){
    ...
    const population=mode==='curve'?curvePopulation():state.plot,values=groupingValues(group,population),colors=seriesColors(group,values);
  ...
  function renderLegend(){
    ...
    if(state.pointGroup===state.curveGroup){
      ...
      legend.append(renderLegendGroup(`回歸曲線｜${groupLabels[state.curveGroup]}`,state.curveGroup,'curve'));
    }
  }
  ...
  function render(resetDomain=true){
    ...
    elements['chart-subtitle'].textContent=`曲線依${groupLabels[state.curveGroup]}分類並套用全部篩選；個券搜尋只影響點位與表格。${elements['show-outliers'].checked?'圖中包含異常值。':'異常值預設不顯示。'}`;
  ...
  function initializeFilters(){
    ...
    state.pointGroup='band';state.curveGroup='band';elements['point-group'].value='band';elements['curve-group'].value='band';elements['grouping-notice'].textContent='';syncCascadingFilters();state.curveKey='';
  }
  ...
  elements['reset-filters'].addEventListener('click',()=>{elements['bond-search'].value='';elements['bond-sort'].value='residual-desc';elements['show-curves'].checked=true;elements['show-outliers'].checked=false;document.querySelectorAll('[data-list-search]').forEach(input=>{input.value='';});document.querySelector('[name=bond-metric][value=yield_pct]').checked=true;state.metric='yield_pct';initializeFilters();state.page=1;render();});
